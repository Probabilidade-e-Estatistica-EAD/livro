...
# Inferência para dados categóricos
...
## Inferência para uma única proporção
...
### Identificar quando a proporção da amostra é quase normal
...
### Escolhendo um tamanho de amostra ao estimar uma proporção
...
### Intervalos de confiança para $p_1-p_2$
...
### Testes de hipóteses para $p_1-p_2$
...
###  Mais sobre testes de hipóteses de duas proporções (tópico especial)
...
## Testando a qualidade do ajuste usando a qui-quadrado (tópico especial)
...
###  Criando uma estatística de teste para tabelas unidirecionais
...
### Encontrar um p-valor para uma distribuição qui-quadrado
...
###  Avaliando a qualidade do ajuste para uma distribuição
...
## Teste de independência em tabelas bidirecionais (tópico especial)
...
### Contagens esperadas em tabelas bidirecionais
...
### O teste qui-quadrado para tabelas bidirecionais
...
### Quando a condição de falha ou sucesso não é atendida 
...
### Gerando a distribuição nula e o p-valor por simulação 
...
### Usando simulação para testes de adequação
...
### Estrutura de amostra grande para uma diferença em duas proporções 
...
### Simulando uma diferença sob a distribuição nula
...
Qual é a diferença nas taxas de sobrevivência entre os dois grupos falsos na Tabela \@ref(tab:resultsForCPRStudyInSmallSampleSectionFake1)? Como isso se compara aos 13\% observados nos grupos reais?^[A diferença é $\hat{p}_{t, falso} - \hat{p}_{c, falso} = \frac{10}{40} - \frac{15}{50} = -0.05$, que está mais próximo do valor nulo $p_0 = 0$ do que o que observamos.]
```
...
### Distribuição nula para a diferença em duas proporções

Construímos uma aproximação para a distribuição nula criando repetidamente tabelas como a mostrada na Tabela \@ref(tab:resultsForCPRStudyInSmallSampleSectionFake1) e calcular as diferenças de amostra. A distribuição nula de 10.000 simulações é mostrada na Figura \@ref(fig:pValueCPRStudySmallSampleAnalysisInSmallSampleSection).
...
### Randomização para tabelas bidirecionais e qui-quadrado
