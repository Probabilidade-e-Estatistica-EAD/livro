...
# Regressão Múltipla e Logística {#multipleAndLogisticRegression}
...
### $R^2$ ajustado como melhor estimativa da variância explicada
...
### Identificando variáveis no modelo que podem não ser úteis
...
table6 <- data.frame(summary(g)$coefficients)
colnames(table6) <- colnames(table5)
table6[5,] <- c(0.7128, '', '', '')
rownames(table6) <- c('(Intercepto)', 'Condição', 'Foto', 'Rodas', 'R² adj')


knitr::kable(table6, digits = 3, 
             caption = 'O ajuste para o modelo de regressão para preditores condição, foto e rodas.')
```
...
### Duas estratégias de seleção de modelo
...
### A abordagem do p-valor, uma alternativa para o $R^2$ ajustado
...
### Decisões práticas no aplicativo de e-mail
...
### Diagnóstico para o classificador de e-mail
