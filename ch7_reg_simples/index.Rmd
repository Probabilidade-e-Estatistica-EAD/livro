...
# Introdução à Regressão Linear {#linRegrForTwoVar}

```{r, include=FALSE}
knitr::opts_chunk$set(warning = FALSE, message = FALSE, 
                      echo = TRUE, cache = TRUE, fig.align = "center",
                      out.width='100%', out.height='30%', fig.height=3)
...
ggplot(data = possum) + 
  theme(panel.border = element_rect(colour = "black", fill = NA, size = 1)) + 
  geom_point(aes(possum$totalL, possum$headL), color = 'skyblue3') + 
  labs(x = 'Comprimento Total (cm)', y = 'Comprimento da Cabeça (cm)') + 
  geom_point(aes(89, 94.1), color = 'red') +
  geom_segment(aes(x = 89, y = min(possum$headL), xend = 89, yend = 94.1), 
               linetype = 'dashed', color = 'red') +
  geom_segment(aes(x = min(possum$totalL), y = 94.1, xend = 89, yend = 94.1), 
               linetype = 'dashed', color = 'red')
...
knitr::include_graphics('brushtail_possum.jpg')
```
...
### Sobrepor uma linha visualmente
...
ggplot() + 
  theme(panel.border = element_rect(colour = "black", fill = NA, size = 1)) + 
  geom_point(data = possum, aes(possum$totalL, possum$headL), color = 'skyblue3') + 
  labs(x = 'Comprimento Total (cm)', y = 'Comprimento da Cabeça (cm)') + 
...
### Descrevendo relacionamentos lineares com correlação
...
### Uma medida objetiva para encontrar a melhor linha
...
### Interpretando estimativas de parâmetros da linha de regressão
...
### Usando $R^2$ para descrever a força de um ajuste
...
\begin{align*}
\widehat{preço} = \beta_0 + \beta_1 \times \text{\var{cond\_\hspace{0.3mm}nova}}
\end{align*}

O modelo ajustado está resumido na Tabela \@ref(fig:marioKartNewUsedRegrSummary), e o modelo com seus parâmetros estimados é dado como

\begin{align*}
\widehat{preço} = 42.87 + 10.90 \times \text{\var{cond\_\hspace{0.3mm}nova}}
\end{align*}
...
### Eleições de meio de mandato e desemprego
